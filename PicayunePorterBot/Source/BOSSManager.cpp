...
void BOSSManager::startNewSearch(const std::vector<MetaPair> & goalUnits)
{
    ...
    // convert from Picayune_Porter's meta goal type to BOSS ActionType goal
    try
    {
...
void BOSSManager::update(double timeLimit)
{
    ...
    if (isSearchInProgress())
    {
        ...
        {
            PiPo_ASSERT_WARNING(false, "BOSS SmartSearch Exception: %s", exception.what());
        ...
        if (previousSearchComplete)
        {
            ...
            if (!solved && _previousBuildOrder.size() == 0)
            {
                ...
                {
                    PiPo_ASSERT_WARNING(false, "BOSS Timeout Naive Search Exception: %s", exception.what());
...
BOSS::ActionType BOSSManager::GetActionType(const MetaType & t)
{
	...
	{
		PiPo_ASSERT(false, "Should have found a valid type here");
	}
