...
void WorkerManager::handleIdleWorkers() 
{
	...
	for (auto & worker : workerData.getWorkers())
	{
        PiPo_ASSERT(worker != nullptr, "Worker was null");
...
{
    PiPo_ASSERT(enemyUnit != nullptr, "enemyUnit was null");
...
{
	PiPo_ASSERT(refinery != nullptr, "Refinery was null");
...
bool WorkerManager::willHaveResources(int mineralsRequired, int gasRequired, double distance)
{
    ...
    PiPo_ASSERT(speed > 0, "Speed is negative");
...
void WorkerManager::drawWorkerInformation(int x, int y) 
{
	...
	for (auto & unit : workerData.getWorkers())
	{
        PiPo_ASSERT(unit != nullptr, "Worker was null");
